...
def main() -> int:
    ...
    query = "\n".join([
        ...
        "请从下面已经生成好的分集分句素材中，设计 10 个不同版本的投放高光剪辑方案。",
        ...
        "",
        "选择要求：",
        "1. 一次输出 10 条 highlights，10 条必须是不同剪辑版本，不能只是按集数顺序切 10 段。",
        "2. 每条高光段控制在 2-3 分钟左右，优先约 100 句。",
        "3. 每条开头约 2 分钟应从全剧/全部分集中挑选最强高光内容，形成强钩子混剪，而不是从第1集顺序开始。",
        "4. 每条开头要覆盖冲突、反转、悬念、身份错位、危机、爽点或强情绪。",
        "5. 每条中后段可以围绕1条主线收束，保证观众能看懂。",
        "6. 每条 shots 数组只返回你选择的句子，字段只需要 episode 和 sentence。",
        "7. 下游会随机选择1-2集连续素材补足到 5-8 分钟，所以这里不要为了凑时长选择弱剧情。",
        "",
    ...
    agent_input = {
        ...
        "agent_payload": {
            ...
            "rules": [
                ...
                "10个组合必须是不同剪辑版本，不能按集数顺序切段",
                "每个组合开头约2分钟从全剧高光内容中挑选，形成强钩子混剪",
                "每个组合约100句，对应2-3分钟高光段",
                "输出10个highlights，每个highlight只需要shots数组",
                "shots字段只保留episode和sentence",
                "后续拼接链路会随机选择1-2集连续素材补足到5-8分钟，避免太跳跃"
            ],
